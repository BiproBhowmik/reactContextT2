...
const CardSection = () => {
    ...
    const InputObject = useContext(InputObj)

    // console.log(InputObject);

    var recevedValueContext = []

    if (InputObject.data.length > 0) {
        console.log(InputObject.data);
        recevedValueContext = InputObject.data
        // setPeople({
        //     data: x
        // })
    }
    ...
    const deleteItem = async (item: { id: any }) => {
    ...
    if (recevedValueContext.length > 0) {
        showPeopleValue = recevedValueContext
    }
    else{
        showPeopleValue = getPeople.data
    }

    const showPeople = showPeopleValue.map((item: any, index: React.Key | null | undefined) => {
